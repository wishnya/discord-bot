...
commands = '\n'.join([
    ':nerd: Чтобы вывести топ-10 местных эрудитов, можно воспользоваться командой "!top". :first_place:',
    ':cat: Чтобы вывести изображение няшного кисика, используйте команду "!cat". :smirk_cat:',
    ':file_cabinet: Чтобы осуществить поиск по википедии, используйте команду "!wiki ", а затем искомое(можно сразу несколько искомых слов через пробел). :books:',
    ':snake: Практически точно так же работает поиск по документации python 3 версии, но с помощью команды "!python". :dragon_face:',
    ':question: Для того, вывести вопрос викторины, используйте команду "!в" на русской раскладке. :thinking:',
    ':speaking_head: Ответ принимается без дополнительных команд и состоит из одного слова. :speech_balloon:'
])
